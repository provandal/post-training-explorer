...
const FALLBACK_LOSS_CURVE = (() => {
  ...
  for (let step = 0; step <= 525; step += 3) {
    ...
    const noise =
      (Math.sin(step * 0.7) * 0.08 + Math.cos(step * 1.3) * 0.05) * Math.exp(-step / 200)
...
function LoRAWeightHeatmap() {
  ...
  useEffect(() => {
    ...
    if (realWeights?.lora_A && realWeights?.lora_B) {
      ...
      for (let i = 0; i < rows; i++) {
        for (let j = 0; j < cols; j++) {
          const signal =
            (i < 4 ? 0.3 : 0.1) * Math.sin(i * 0.8 + j * 0.3) * Math.cos(j * 0.5 - i * 0.2)
    ...
    svg
      .append('text')
      .attr('x', margin.left + (cols * cellSize) / 2)
      .attr('y', margin.top + rows * cellSize + 20)
      .attr('text-anchor', 'middle')
      .attr('fill', '#64748b')
      .attr('font-size', '9')
    ...
    svg
      ...
      .attr('x', -(margin.top + (rows * cellSize) / 2))
      .attr('y', 15)
      .attr('text-anchor', 'middle')
      .attr('fill', '#64748b')
      .attr('font-size', '9')
    ...
    const legendG = svg
      .append('g')
      .attr('transform', `translate(${margin.left + cols * cellSize + 15}, ${margin.top})`)
    const legendScale = d3
      .scaleLinear()
      .domain([-maxAbs, maxAbs])
      .range([rows * cellSize, 0])
    const legendAxis = d3.axisRight(legendScale).ticks(5).tickFormat(d3.format('.1f'))

    const defs = svg.append('defs')
    const gradient = defs
      .append('linearGradient')
      .attr('id', 'lora-gradient')
      .attr('x1', '0')
      .attr('y1', '1')
      .attr('x2', '0')
      .attr('y2', '0')
  ...
  if (hasRealData) {
    ...
    const realExamples = testPrompts.slice(0, 5).map((tp) => {
      ...
      return {
        ...
        baseCorrect: baseOut
          ? baseOut.generated_text.toLowerCase().includes(tp.true_label.toLowerCase())
          : false,
        sftCorrect: sftOut
          ? sftOut.generated_text.toLowerCase().includes(tp.true_label.toLowerCase())
          : false,
      }
    })
        ...
        <div className="space-y-5">
          ...
          <div className="p-4 rounded-lg bg-gradient-to-r from-yellow-950/30 via-slate-800/50 to-violet-950/30 border border-violet-700/30">
            <p className="text-sm text-slate-300 leading-relaxed">
              ...
              But the model's behavior &mdash; its format, confidence style, and conciseness &mdash;
              stays the same. To change how the model <em>acts</em>, we need to change the model
              itself. That means modifying its weights through training.
            </p>
            <p className="text-sm text-slate-300 mt-2 leading-relaxed">
              This is where{' '}
              <span className="font-bold text-violet-400">Supervised Fine-Tuning (SFT)</span> comes
              in. We are crossing from the left side of the map (prompt engineering, RAG) to the
              right side (post-training). From here on, we are changing the model.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-5">
          <div className="p-5 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <div className="p-3 rounded bg-violet-950/20 border border-violet-800/30">
              <p className="text-sm text-violet-300 italic leading-relaxed">
                Think of it like training a new employee with an example handbook. You hand them
                1,400 examples that say: "When you see this I/O pattern, respond exactly like this."
                After studying the handbook, they internalize the patterns and can handle new cases
          ...
          <div className="p-5 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <p className="text-xs text-slate-400 mb-4 leading-relaxed">
              ...
              (what we want the model to classify). The output is the structured response we want
              the model to produce. Here is one of the 1,400 examples:
            </p>
...
processing.`}
            ...
            <p className="text-xs text-slate-500 mt-3 leading-relaxed">
              The dataset contains 1,400 examples across 6 workload categories (OLTP, OLAP, VDI,
              Backup, AI/ML Training, Video Streaming) with varying difficulty levels.
            </p>
          </div>
          ...
          <div className="p-5 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-3">
              ...
              significant GPU memory, time, and storage.{' '}
              <strong className="text-violet-300">LoRA (Low-Rank Adaptation)</strong> is a technique
              that freezes the entire original model and trains only a small set of adapter weights
            ...
            <div className="p-3 rounded bg-blue-950/20 border border-blue-800/30 mb-4">
              <p className="text-sm text-blue-300 italic leading-relaxed">
                Instead of rewriting the entire model (360M parameters), we only train a small
                adapter (<strong>432K parameters &mdash; 0.12%</strong> of the total). Think of it
                like applying a firmware patch rather than reflashing the entire image. The base
                model stays intact; you just overlay a tiny delta that changes the behavior.
              </p>
            </div>
            ...
            <p className="text-xs text-slate-400 leading-relaxed mb-2">
              ...
              forward. By adapting just these projections with a rank-16 decomposition, LoRA
              captures the essential behavioral change in a tiny number of parameters.
            </p>
            <button
            ...
            {showLoRAAside && (
              <div className="mt-2 p-3 rounded bg-violet-950/20 border border-violet-800/30">
                <p className="text-xs text-slate-300 leading-relaxed mb-2">
                  LoRA doesn't search for parameters &mdash; you configure which weight matrices to
                  target. The standard choice is the{' '}
                  <strong className="text-violet-300">query and value projections</strong> (q_proj,
                  v_proj) in each attention layer, because research shows these have the most impact
                  on model behavior. The adapter itself is a pair of small matrices (A and B) whose
                  product approximates the weight change that full fine-tuning would make. The
                  "rank" (16 in our case) controls how expressive the adapter is &mdash; higher rank
                  = more capacity, but more parameters.
                </p>
                <button
                  onClick={() => {
                ...
                >
                  Deep dive: LoRA parameter selection, rank decomposition, and the training loop
                  &rarr;
                </button>
              </div>
            )}
            <p className="text-xs text-slate-500 leading-relaxed mt-3">
              During inference, the adapter weights are merged with the base model at near-zero
              cost. You can even swap different adapters for different tasks on the same base model
              &mdash; like hot-swapping firmware modules.
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-gradient-to-r from-violet-950/30 to-blue-950/30 border border-violet-700/30">
            <p className="text-xs text-blue-300 leading-relaxed">
              <strong>Key insight:</strong> SFT changes the model's weights. After training, the
              model permanently knows how to classify I/O patterns &mdash; no prompt engineering
              needed. You don't need to tell it the format, the categories, or the reasoning style
              every time. It learned all of that from the 1,400 examples. The adapter is a 1.7 MB
              file that gets merged at load time, and the model behaves differently from that point
              forward.
            </p>
          </div>
        </div>
        ...
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-center">
              ...
              <div>
                <div className="text-lg font-bold text-violet-400">
                  {sftSummary
                    ? `${(sftSummary.accuracy * 100).toFixed(0)}%`
                    : `${trainingTimeMin} min`}
                </div>
                <div className="text-xs text-slate-500">
                  {sftSummary
                    ? `accuracy (${sftSummary.correct}/${sftSummary.total})`
                    : 'training time'}
                </div>
              </div>
            </div>
          ...
          <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            <p className="text-sm text-slate-300 mb-2 leading-relaxed">
              <span className="font-semibold text-violet-400">What changed?</span> The base model
              sees the same metrics and produces vague, hedging descriptions. It never commits to a
              classification, never uses a structured format, and often gets the category wrong
              entirely.
            </p>
            <p className="text-sm text-slate-300 leading-relaxed">
              After SFT, the model has learned three things:{' '}
              <strong className="text-slate-200">the format</strong> (Classification / Confidence /
              Key indicators), <strong className="text-slate-200">the task</strong> (commit to a
              specific workload category), and{' '}
              <strong className="text-slate-200">the reasoning style</strong> (cite the specific
              metrics that led to the decision). All from 1,400 examples and {trainingTimeMin}{' '}
              minutes of training.
            </p>
          </div>
        </div>
      ...
      {section === 'deepdive' && (
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            <p className="text-sm text-slate-300 leading-relaxed">
              SFT modifies the model's internal weights so it produces different outputs. But what
              does "different" look like at a technical level? These three views show what actually
              changed inside the model during training.
            </p>
          </div>
          ...
          {deepTab === 'probs' && (
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              ...
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                ...
                the model generates. Before SFT, probability is spread across generic words ("This",
                "The", "Based"). After SFT, probability mass concentrates on structured tokens like
                "Classification".
              </p>
              <TokenProbChart
                data={baseProbs}
            ...
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              ...
              {showTransformerAside && (
                <div className="mb-4 p-3 rounded bg-violet-950/20 border border-violet-800/30">
                  <p className="text-xs text-slate-300 leading-relaxed mb-2">
                    Language models are built from{' '}
                    <strong className="text-violet-300">Transformers</strong> &mdash; a stack of
                    identical layers (SmolLM2 has 32). Each layer contains an{' '}
                    <strong className="text-violet-300">attention mechanism</strong> that lets every
                    token in the input "look at" every other token to decide what's relevant. This
                    is how the model connects "IOPS: 45000" with "OLTP Database" even when they're
                    far apart. The attention mechanism uses learned weight matrices (Q, K, V)
          ...
          {deepTab === 'loss' && (
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              ...
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                ...
                {lossCurve.length > 0 ? lossCurve[lossCurve.length - 1].step : 525} optimization
                steps. <strong className="text-slate-200">Loss</strong> measures how far the model's
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <div className="p-3 rounded bg-slate-800/80 border border-slate-700/50">
                  ...
                  <p className="text-xs text-slate-400 mt-1 leading-relaxed">
                    ...
                    {lossCurve.length > 0
                      ? lossCurve[lossCurve.length - 1].loss.toFixed(2)
                      : '0.25'}
                    . Going lower would risk overfitting.
                  </p>
                </div>
                <div className="p-3 rounded bg-slate-800/80 border border-slate-700/50">
                  <div className="text-xs text-slate-500">
