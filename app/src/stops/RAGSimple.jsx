        ...
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-red-950/20 border border-red-800/30">
            ...
            <p className="text-sm text-slate-300 mb-3">
              In the previous stop, we added examples to the prompt and the model improved.
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <div className="p-3 rounded bg-slate-800/50">
                <h4 className="text-sm font-semibold text-red-300 mb-1">Can't scale to real knowledge</h4>
                <p className="text-xs text-slate-400">
          ...
          <div className="p-4 rounded-lg bg-yellow-950/20 border border-yellow-800/30">
            <p className="text-sm text-slate-300">
              <span className="font-semibold text-yellow-400">What if the model could look things up?</span>{' '}
              Instead of cramming everything into the prompt, what if the model had access to a
              reference library — and could search it for just the relevant information before answering?
            </p>
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 mb-4">
              ...
              <strong className="text-yellow-300"> searches a knowledge base</strong> for information
              relevant to the current query and includes it in the prompt automatically.
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700/50">
              ...
              <div className="p-2 rounded bg-slate-900 font-mono text-xs text-slate-500">
                "High IOPS, small blocks" → [0.82, -0.14, 0.67, 0.31, ...]<br />
                "Many random reads, 4K" → [0.79, -0.11, 0.71, 0.28, ...]<br />
            ...
            <div className="p-4 rounded-lg bg-slate-800/50 border border-slate-700/50">
              ...
              <p className="text-xs text-slate-400 mb-2">
                Your query is embedded into the same vector space as the knowledge base.
                Then we find the nearest neighbors — the entries whose vectors are most similar
                to your query's vector. This is "semantic search": it finds relevant content
                even if the exact words don't match.
              </p>
              <div className="p-2 rounded bg-slate-900 font-mono text-xs text-slate-500">
                cosine_similarity(query, doc) = (q · d) / (|q| × |d|)<br />
          ...
          <div className="p-3 rounded bg-blue-950/20 border border-blue-800/30">
            <p className="text-xs text-blue-300">
              <strong>Key insight:</strong> RAG does not change the model's weights at all.
              The model is identical — only the input it receives is different. RAG changes
              what the model <em>sees</em>, not how it <em>thinks</em>.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 mb-4">
              ...
              (hundreds of numbers). When your query arrives, it's also embedded. Then we find
              which stored vectors are closest to the query vector. Here's what that looks like
              in 2D (real embeddings have hundreds of dimensions, but the clustering principle is the same):
            </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
              ...
              <p className="text-xs text-slate-400">
                ...
                "high IOPS with small blocks" and "transaction processing database" are
                semantically related — so they get similar vectors. OLTP entries cluster
                together, backup entries cluster together, etc. This happens automatically
            ...
            <div className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/50">
              ...
              <p className="text-xs text-slate-400">
                The embedding vectors are stored in a <strong>vector database</strong> (e.g., Pinecone,
                Milvus, pgvector). For our 13-entry demo, an in-memory array works fine. At enterprise
                scale with millions of entries, you need optimized approximate nearest neighbor (ANN)
                indexes — and that means dedicated storage and compute for the vector DB.
              </p>
            </div>
          </div>
          ...
          <div className="p-3 rounded bg-yellow-950/20 border border-yellow-800/30">
            <h4 className="text-xs font-semibold text-yellow-400 mb-1">Where storage fits in RAG</h4>
            <div className="grid grid-cols-3 gap-3 mt-2 text-xs text-slate-400">
              <div>
                <span className="font-semibold text-slate-300">Embedding indexes</span><br />
                ~1.5 KB per entry (768-dim float32). 1M entries ≈ 1.5 GB.
              </div>
              <div>
                <span className="font-semibold text-slate-300">Source documents</span><br />
                The original text. Often larger than the embeddings themselves.
              </div>
              <div>
                <span className="font-semibold text-slate-300">Query latency</span><br />
