...
const FALLBACK_COMPARISON = {
  ...
  accuracy_by_technique: {
    base:  { "360M": 0.00, "1.7B": 0.10 },
    sft:   { "360M": 0.65, "1.7B": 0.80 },
    dpo:   { "360M": 0.60, "1.7B": 0.75 },
  ...
  head_to_head: [
    {
      prompt_id: 0,
      prompt_snippet: "IOPS: 45,000 | Throughput: 180 MB/s | Latency: 850 us | R/W: 72/28 | Random: 93% | Block: 8 KB | QD: 64",
      true_label: "OLTP Database",
      results: {
        "360M": {
          sft: { predicted: "OLTP Database", text: "Classification: OLTP Database\nReason: High random IOPS with small block sizes indicate transactional database operations." },
          grpo: { predicted: "OLTP Database", text: "Classification: OLTP Database\nReason: Small block random reads dominate, consistent with index lookups and row fetches." }
        },
        "1.7B": {
    ...
    {
      prompt_id: 1,
      prompt_snippet: "IOPS: 1,200 | Throughput: 2,800 MB/s | Latency: 8,500 us | R/W: 95/5 | Random: 12% | Block: 512 KB | QD: 8",
      true_label: "OLAP Analytics",
      results: {
        "360M": {
          sft: { predicted: "OLAP Analytics", text: "Classification: OLAP Analytics\nReason: Large sequential reads with high throughput indicate analytical table scans." },
          grpo: { predicted: "OLAP Analytics", text: "Classification: OLAP Analytics\nReason: Predominantly sequential read pattern with large blocks suggests data warehouse queries." }
        },
        "1.7B": {
    ...
    {
      prompt_id: 3,
      prompt_snippet: "IOPS: 500 | Throughput: 1,500 MB/s | Latency: 6,000 us | R/W: 97/3 | Random: 18% | Block: 1024 KB | QD: 4",
      true_label: "Video Streaming",
      results: {
        "360M": {
          sft: { predicted: "OLAP Analytics", text: "Classification: OLAP Analytics\nReason: Large sequential reads with high throughput indicate analytical table scans." },
          grpo: { predicted: "Video Streaming", text: "Classification: Video Streaming\nReason: Steady sequential reads with large block sizes indicate media streaming operations." }
        },
        "1.7B": {
    ...
    {
      prompt_id: 4,
      prompt_snippet: "IOPS: 15,000 | Throughput: 90 MB/s | Latency: 1,200 us | R/W: 58/42 | Random: 88% | Block: 8 KB | QD: 32",
      true_label: "VDI Virtual Desktop",
      results: {
        "360M": {
          sft: { predicted: "OLTP Database", text: "Classification: OLTP Database\nReason: High random IOPS with small block sizes indicate transactional database operations." },
          grpo: { predicted: "VDI Virtual Desktop", text: "Classification: VDI Virtual Desktop\nReason: Mixed read/write random I/O with small blocks indicates virtual desktop user activity." }
        },
        "1.7B": {
          sft: { predicted: "VDI Virtual Desktop", text: "Classification: VDI Virtual Desktop\nReason: Balanced read/write ratio with high random IOPS and small blocks indicate multiple concurrent desktop sessions." },
    ...
    <div className="space-y-6">
      <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
        ...
        <p className="text-sm text-slate-300 leading-relaxed mb-4">
          A model's parameter count directly determines its capacity to learn patterns.
          More parameters mean more internal "knobs" the model can tune during training,
          which translates to better ability to distinguish subtle differences between
          storage I/O workload categories. A 1.7B-parameter model has roughly 4.7x the
          capacity of a 360M model — and that extra capacity shows up in accuracy,
          especially on ambiguous patterns where the smaller model struggles.
        </p>
        <p className="text-sm text-slate-300 leading-relaxed">
          But bigger models come with real infrastructure costs: more VRAM, longer training
          times, and larger checkpoints. The question for storage and compute teams is not
          "which is better?" — it's "where is the cost-accuracy tradeoff right for my use case?"
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 text-center">
          <div className="text-3xl font-bold text-emerald-400 mb-1">
            +{Math.round(((data.accuracy_by_technique?.grpo?.["1.7B"] ?? 0.95) - (data.accuracy_by_technique?.grpo?.["360M"] ?? 0.85)) * 100)}%
          </div>
          <div className="text-xs text-slate-400 mb-2">Accuracy Delta (GRPO)</div>
          <div className="text-sm text-slate-300">
            Bigger model = better accuracy baseline
          </div>
        </div>
      </div>
      ...
      <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
        ...
        <p className="text-sm text-slate-300 leading-relaxed mb-3">
          Moving from 360M to 1.7B parameters has cascading effects on infrastructure.
          GPU memory requirements jump from ~3.2 GB to ~12.5 GB — meaning you shift from
          "runs on any GPU" to "needs a mid-range or better card." Training times increase
          4-5x across all techniques, which means longer GPU reservations and higher compute
          costs.
        </p>
        <p className="text-sm text-slate-300 leading-relaxed">
          For storage teams, larger models also mean larger checkpoints saved more frequently,
          higher I/O throughput during data loading, and more intermediate artifacts to manage.
          The decision of which model size to use is fundamentally a capacity planning question
          that bridges ML engineering and infrastructure operations.
        </p>
      </div>
    </div>
  )
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className={`p-4 rounded-lg border ${
          ...
          <div className="text-xs text-slate-400 mb-1">
            Predicted: <span className={correct360 ? 'text-emerald-300' : 'text-red-300'}>{results360?.predicted ?? 'N/A'}</span>
          </div>
        ...
        <div className={`p-4 rounded-lg border ${
          ...
          <div className="text-xs text-slate-400 mb-1">
            Predicted: <span className={correct17B ? 'text-emerald-300' : 'text-red-300'}>{results17B?.predicted ?? 'N/A'}</span>
          </div>
          <pre className="text-xs font-mono whitespace-pre-wrap leading-relaxed text-slate-200 mt-2 p-3 rounded bg-slate-800/60">
      ...
      <div className="p-3 rounded-lg bg-cyan-950/20 border border-cyan-800/30">
        <p className="text-xs text-slate-400 leading-relaxed">
          <strong className="text-cyan-300">Notice:</strong> The 1.7B model tends to produce
          more detailed reasoning and is less likely to confuse similar workload categories.
          On ambiguous patterns (like AI/ML vs Video Streaming), the extra capacity helps
          the larger model distinguish subtle differences in the metrics.
        </p>
      </div>
    </div>
  )
...
function ScalingAnalysis({ data }) {
  ...
  useEffect(() => {
    ...
    // Legend
    const legend = svg.append('g').attr('transform', `translate(${width - 140}, ${margin.top - 20})`)
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
        ...
        <p className="text-xs text-slate-500 mt-3">
          Across all techniques, the 1.7B model consistently outperforms the 360M model.
          The gap narrows with GRPO, suggesting that reinforcement learning with verifiable
          rewards helps smaller models punch above their weight.
        </p>
      </div>
      ...
      <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
        ...
        <div className="grid grid-cols-3 gap-4">
          {trainingTechniques.map(tech => {
            ...
            const multiplier = (typeof time360 === 'number' && typeof time17B === 'number')
              ? `${(time17B / time360).toFixed(1)}x`
              : '—'
            return (
              <div key={tech} className="p-3 rounded-lg bg-slate-800/50 border border-slate-700/30 text-center">
        ...
        <p className="text-xs text-slate-500 mt-3">
          GRPO on 1.7B takes ~150 minutes — generating 8 completions per prompt with a
          larger model is compute-intensive. For infrastructure teams, this means planning
          for sustained GPU reservations and higher I/O throughput during training data loading.
        </p>
      </div>
    </div>
  )
