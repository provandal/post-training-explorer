...
import {
  isLoaded,
  getGRPOGenerationLogs,
  getGRPOAccuracyCurve,
  getGRPORewardCurve,
  getTrainingTime,
...
const GRPO_EXAMPLE = {
  input:
    'IOPS: 12000 | Latency: 4.5ms | Block Size: 128K | Read/Write: 92/8 | Sequential: 78% | Queue Depth: 8',
...
const GRPO_INFRA = {
  ...
  storageIOPattern:
    'Heavy compute bursts (generating 8 completions per prompt), periodic checkpoint writes',
...
function AdvantageWaterfall() {
  ...
  useEffect(() => {
    ...
    generations.forEach((g) => {
      g.advantage = g.reward - mean
    })

    const x = d3
      .scaleBand()
      .domain(generations.map((g) => g.id))
      .range([0, w])
      .padding(0.2)
...
export default function GRPOGenerations() {
  ...
  const activeStdReward = Math.sqrt(
    activeRewards.reduce((sum, r) => sum + (r - activeMeanReward) ** 2, 0) / activeRewards.length,
  )
        ...
        <div className="space-y-6">
          <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-4">
              ...
              pipeline. But it still required{' '}
              <strong className="text-pink-400">400 human-labeled preference pairs</strong>. Someone
              had to look at output pairs and decide which was better. For our storage I/O
              classifier, that meant an engineer spending hours reviewing outputs, comparing styles,
              and clicking "this one is better."
            </p>
          ...
          <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-4">
              ...
              of them: "OLAP Analytics" is either the correct label or it isn't. We don't need a
              human to tell us that &mdash; we can check programmatically. Math problems, code
              correctness, factual lookups &mdash; all of these have
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              ...
              . Instead of collecting human preferences, we let the model generate multiple
              attempts, check which ones are correct, and use that signal to improve. The algorithm
              that makes this work is called <strong className="text-emerald-400">GRPO</strong>{' '}
              &mdash; Group Relative Policy Optimization &mdash; and it's the technique behind
              DeepSeek R1.
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-emerald-950/20 border border-emerald-800/30">
            <p className="text-sm text-emerald-300 leading-relaxed">
              <strong>Can the model learn from its own attempts?</strong> That's GRPO. Head to the
              next tab to see how it works.
            </p>
          </div>
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
            ...
            <p className="text-sm text-slate-300 leading-relaxed mb-4">
              ...
              <strong>generates multiple attempts</strong> for the same input,{' '}
              <strong>scores them with a verifiable reward</strong> (did it get the right answer?),
              and uses the <strong>group statistics as a baseline</strong> to figure out which
              attempts were above-average and which were below.
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-5">
              <div className="p-4 rounded-lg bg-emerald-950/15 border border-emerald-800/30">
                ...
                <p className="text-xs text-slate-400">
                  ...
                  differently, with varying confidence and reasoning.
                </p>
              </div>
              <div className="p-4 rounded-lg bg-emerald-950/15 border border-emerald-800/30">
                ...
                <p className="text-xs text-slate-400">
                  ...
                  <strong>wrong = 0</strong>. No reward model, no human labeler. The answer key is
                  the teacher.
                </p>
              </div>
              <div className="p-4 rounded-lg bg-emerald-950/15 border border-emerald-800/30">
            ...
            <div className="p-4 rounded bg-slate-800 border border-slate-700/50 mb-5">
              <p className="text-sm text-slate-300 leading-relaxed">
                <strong className="text-emerald-300">Key difference from PPO:</strong> Traditional
                RLHF with PPO requires a separately-trained critic network and a separate reward
                model &mdash; both expensive to train and maintain. GRPO replaces both with{' '}
                <strong>group statistics</strong>: just compute the mean and standard deviation of
                the group's rewards. This means roughly{' '}
                <strong className="text-emerald-300">50% less compute</strong> compared to PPO, with
                no extra models to train.
              </p>
            </div>
            ...
            <div className="p-4 rounded bg-slate-800 border border-slate-700/50 mb-5">
              ...
              <div className="text-sm text-slate-400 space-y-2 leading-relaxed">
                <p>
                  <strong className="text-slate-300">In plain English:</strong> For each generation
                  in the group, subtract the group's mean reward. If you got a reward of 1.0 and the
                  group mean is 0.5, your advantage is positive &mdash; you did better than average.
                  If you got 0.0, your advantage is negative &mdash; you did worse. Dividing by
                  standard deviation normalizes the scores so the model makes meaningful gradient
                  updates.
                </p>
                <p>
                  <span className="text-emerald-300 font-semibold">Positive advantage</span> &rarr;
                  increase the probability of generating this response.
                  <br />
                  <span className="text-red-300 font-semibold">Negative advantage</span> &rarr;
                  decrease the probability of generating this response.
                </p>
              </div>
            </div>
            ...
            <div className="p-4 rounded bg-slate-800 border border-slate-700/50 mb-5">
              <p className="text-sm text-slate-300 leading-relaxed">
                <strong className="text-emerald-300">Analogy:</strong> Imagine a study group where
                everyone answers the same exam question, then they grade each other's homework. The
                answer key is the teacher &mdash; no human grader needed. Students who scored above
                the group average get encouragement ("do more of this"), and those who scored below
                get correction ("do less of this"). Over time, the whole group converges on
                consistently right answers.
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg bg-blue-950/20 border border-blue-800/30">
            <p className="text-sm text-blue-300 leading-relaxed">
              <strong>Key insight:</strong> GRPO needs no human labels and no reward model. The
              answer itself is the teacher. For any task with a verifiable correct answer &mdash;
              classification, math, code &mdash; the model can improve itself purely from its own
              attempts.
            </p>
          </div>
          ...
          <div className="p-5 rounded-lg bg-slate-800/40 border border-slate-700/50">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
              ...
              <div className="p-4 rounded-lg bg-emerald-950/30 border border-emerald-800/30">
                <div className="text-2xl font-bold text-emerald-400">
                  {grpoTrainingTime ? `${Math.round(grpoTrainingTime / 60)} min` : '35 min'}
                </div>
            ...
            <p className="text-xs text-slate-500 mt-3">
              GRPO takes longer per run because it generates 8 completions per prompt (8x the
              compute per step). But it eliminates the hours of human labeling that DPO requires,
              making it far more scalable to new tasks.
            </p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-emerald-950/20 border border-emerald-800/30">
            ...
            <p className="text-sm text-slate-300 leading-relaxed">
              GRPO is powerful, but it only works when you can <em>automatically check</em> if an
              answer is correct. This class of problems is called{' '}
              <strong>Reinforcement Learning with Verifiable Rewards (RLVR)</strong>. Examples:
            </p>
            <ul className="text-sm text-slate-400 mt-2 space-y-1 list-disc list-inside">
              <li>
                <strong>Our task:</strong> Does "Classification: OLAP Analytics" match the ground
                truth label? Verifiable.
              </li>
              <li>
              ...
              <li>
                <strong className="text-slate-500">Creative writing:</strong> Is this poem "good"?{' '}
                <em>Not</em> verifiable &mdash; you'd still need DPO or RLHF for this.
              </li>
            </ul>
            <p className="text-xs text-slate-500 mt-3 italic">
              RLVR is what makes GRPO practical. For any task with a clear right/wrong answer, you
              can skip human labeling entirely.
            </p>
          </div>
        </div>
      )}
      ...
      {section === 'demo' && (
        <div className="space-y-4">
          <div className="p-4 rounded-lg bg-slate-800/40 border border-slate-700/50 mb-2">
            <p className="text-sm text-slate-300 leading-relaxed">
              Below is an ambiguous I/O pattern that could plausibly be OLAP Analytics or AI/ML
              Training. The model generates <strong className="text-emerald-400">8 attempts</strong>
              . Each is automatically scored: correct classification = 1.0, incorrect = 0.0. Reveal
              them one at a time or all at once to see how the group splits.
            </p>
          </div>
          ...
          <div className="mb-4">
            ...
            <p className="text-xs text-slate-500 mt-1">
              Correct answer:{' '}
              <span className="text-emerald-400 font-semibold">{activeExample.correctLabel}</span> |
              The model generates <strong>{activeExample.generations.length} attempts</strong>. Each
              is scored: correct = 1.0, incorrect = 0.0.
            </p>
          </div>
          ...
          {showStats && (
            <div className="p-4 rounded-lg bg-emerald-950/20 border border-emerald-800/30">
              ...
              <p className="text-xs text-slate-400">
                <strong className="text-emerald-300">How GRPO learns:</strong> The mean reward (
                {activeMeanReward.toFixed(3)}) is the baseline. The {correctCount} correct
                generations get <span className="text-emerald-300">positive advantage</span> (+
                {(1.0 - activeMeanReward).toFixed(2)}) &mdash; they're reinforced. The{' '}
                {activeExample.generations.length - correctCount} incorrect generations get{' '}
                <span className="text-red-300">negative advantage</span> (
                {(0 - activeMeanReward).toFixed(2)}) &mdash; they're suppressed. No critic network,
                no reward model &mdash; just group statistics.
              </p>

              <p className="text-xs text-slate-500 mt-2 italic">
                This is the technique behind DeepSeek R1. The answer itself is the teacher. For our
                task, "correct classification" is verifiable. For math, "correct answer" is
                verifiable. That's why it's called Reinforcement Learning with Verifiable Rewards
                (RLVR).
              </p>
            </div>
          )}
      ...
      {section === 'deepdive' && (
        <div className="space-y-6">
          ...
          {vizTab === 'advantage' && (
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                ...
                Correct classifications (green) get{' '}
                <strong className="text-emerald-400">positive advantage</strong> &mdash; they're
                reinforced. Incorrect ones (red) get{' '}
                <strong className="text-red-400">negative advantage</strong> &mdash; they're
                suppressed. The model learns to generate more responses like the winners.
              </p>
              <AdvantageWaterfall />
              <p className="text-xs text-slate-500 mt-3">
                Notice: no external reward model, no human labels. The advantage is computed
                entirely from the group's own statistics. This is what makes GRPO 50% cheaper than
                PPO.
              </p>
            </div>
          )}
          ...
          {vizTab === 'accuracy' && (
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                ...
                successes and failures. Starting at ~45% (random-ish), reaching ~85% after 200
                steps. Each step involves generating 8 completions, scoring them, and updating the
                model toward the correct ones.
              </p>
              <LossChart
                data={accuracyCurve}
          ...
          {vizTab === 'reward' && (
            <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
              <p className="text-sm text-slate-300 mb-4 leading-relaxed">
                Mean group reward increases as more generations become correct. This curve directly
                reflects the model's improving ability to classify I/O patterns. A mean reward of
                1.0 would mean every generation in every group is correct.
              </p>
              <LossChart
                data={rewardCurve}
          ...
          <div className="p-4 rounded bg-slate-800 border border-slate-700/50">
            ...
            <p className="text-xs text-slate-500 mt-3 leading-relaxed">
              <strong className="text-slate-400">Reading the formula:</strong> The advantage tells
              the model how much better (or worse) each generation was compared to the group.
              Multiplying by log_prob means "adjust the probability of generating this response by
              this much." Positive advantage = reinforce. Negative = suppress. The std normalization
              prevents the model from only making tiny updates.
            </p>
          </div>
