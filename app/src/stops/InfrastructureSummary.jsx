...
function buildTechniques() {
  ...
  return [
    ...
    },
    {
      ...
      models: 1,
      color: '#eab308',
      storage: 'Vector DB: ~2-10 GB for embeddings',
    },
    {
      name: 'SFT\n(LoRA)',
      gpu: 4.2,
      time: sftTime ? Math.round(sftTime / 60) : 12,
      cost: 3,
      models: 1,
      color: '#8b5cf6',
    ...
    },
    {
      name: 'DPO\n(LoRA)',
      gpu: 5.1,
      time: dpoTime ? Math.round(dpoTime / 60) : 8,
      cost: 4,
      models: 1,
      color: '#ec4899',
    ...
    },
    {
      name: 'GRPO',
      gpu: 6.8,
      time: grpoTime ? Math.round(grpoTime / 60) : 35,
      cost: 7,
      models: 1,
      color: '#10b981',
...
function ComparisonChart({ metric, techniques }) {
  ...
  useEffect(() => {
    ...
    const x = d3
      .scaleBand()
      .domain(TECHNIQUES.map((t) => t.name))
      .range([0, w])
      .padding(0.3)
...
function StorageFootprintChart({ profile }) {
  ...
  useEffect(() => {
    ...
    g.selectAll('.grid')
      .data(ticks.filter((t) => t <= maxVal * 1.5))
      .join('line')
      .attr('x1', (d) => x(d))
      .attr('x2', (d) => x(d))
      .attr('y1', 0)
  ...
  useEffect(() => {
    ...
    techniqueNames.forEach((tech) => {
      ...
      events.forEach((evt, i) => {
        ...
        g.append('rect')
          ...
          .attr('height', barH)
          .attr('fill', color)
          .attr('opacity', evt.direction === 'compute' ? 0.3 : 0.7)
          .attr('rx', i === 0 ? 3 : 0)
          .transition()
          .duration(500)
          .delay(i * 80)
        ...
        if (evt.size_mb > 0 && evt.duration_pct >= 10) {
          g.append('text')
            ...
            .attr('font-size', '7')
            .text(
              evt.size_mb >= 100 ? `${(evt.size_mb / 1000).toFixed(1)} GB` : `${evt.size_mb} MB`,
            )
            .attr('opacity', 0)
            .transition()
            .delay(600 + i * 80)
    ...
    legendData.forEach((d, i) => {
      legend
        ...
        .attr('height', 10)
        .attr('fill', d.color)
        .attr('opacity', d.label === 'Compute' ? 0.3 : 0.7)
        .attr('rx', 2)
      legend
        .append('text')
        .attr('x', i * 80 + 14)
  ...
  useEffect(() => {
    ...
    g.selectAll('.local-label')
      ...
      .text((d) => {
        const v = d.local
        return v >= 1000
          ? `${(v / 1000).toFixed(1)}s local`
          : `${v < 1 ? '<1' : v.toFixed(0)}ms local`
      })
      .attr('opacity', 0)
      .transition()
      .delay(800)
          ...
          {sizes.map((size) => (
            <tr key={size} className="border-b border-slate-800/50">
              ...
              <td className="py-2 px-3 text-right text-slate-400">
                {fmt(scaling[size].base_model_mb)}
              </td>
              <td className="py-2 px-3 text-right text-slate-400">
                {fmt(scaling[size].full_finetune_checkpoint_mb)}
              </td>
              <td className="py-2 px-3 text-right text-violet-300 font-semibold">
                {fmt(scaling[size].lora_adapter_mb)}
              </td>
              <td className="py-2 pl-3 text-right text-emerald-400 font-semibold">
                {scaling[size].storage_reduction_pct}%
              </td>
            </tr>
          ))}
        </tbody>
...
export default function InfrastructureSummary() {
  ...
  const fallbackProfile = {
    ...
    techniques: {
      sft: {
        adapter_size_mb: 1.7,
        adapter_pct_of_base: 0.236,
        full_checkpoint_size_mb: 720,
        dataset_read_mb: 0.74,
        epochs: 5,
        checkpoint_saves: 5,
        total_write_mb: 8.5,
        total_read_mb: 723.7,
        training_time_seconds: 394,
        io_events: [
          {
            phase: 'Model Load',
            direction: 'read',
          ...
          {
            phase: 'Dataset Scan (5 epochs)',
            direction: 'read',
            size_mb: 3.7,
            pattern: 'sequential_repeated',
            duration_pct: 85,
          },
          {
            phase: 'Checkpoint Saves (5x)',
            direction: 'write',
            size_mb: 8.5,
      ...
      dpo: {
        ...
        adapter_pct_of_base: 0.236,
        full_checkpoint_size_mb: 720,
        dataset_read_mb: 1.48,
        epochs: 2,
        checkpoint_saves: 2,
        total_write_mb: 3.4,
        total_read_mb: 727.6,
        training_time_seconds: 317,
        io_events: [
          {
            phase: 'Base + SFT Load',
            direction: 'read',
          ...
          {
            phase: 'Preference Pairs (2 epochs)',
            direction: 'read',
            size_mb: 2.96,
            pattern: 'sequential_repeated',
            duration_pct: 75,
      ...
      grpo: {
        ...
        adapter_pct_of_base: 0.236,
        full_checkpoint_size_mb: 720,
        dataset_read_mb: 0.22,
        epochs: 2,
        checkpoint_saves: 2,
        generations_per_prompt: 8,
        total_write_mb: 3.4,
        total_read_mb: 721.9,
        training_time_seconds: 770,
        io_events: [
          {
            phase: 'Base + SFT Load',
            direction: 'read',
          ...
          {
            phase: '8x Generation Bursts',
            direction: 'compute',
            size_mb: 0,
            pattern: 'burst_compute',
            duration_pct: 60,
          },
          {
            phase: 'Reward + Policy Update',
            direction: 'compute',
            size_mb: 0,
            pattern: 'in_memory',
            duration_pct: 28,
    ...
    scaling_projections: {
      '360M': {
        base_model_mb: 720,
        lora_adapter_mb: 1.7,
        full_finetune_checkpoint_mb: 720,
    ...
    storage_architecture: {
      model_load: {
        local_ssd_seconds: 0.2,
        network_nfs_seconds: 3.6,
        size_mb: 720,
        label: 'Base model load (720 MB)',
      },
      adapter_checkpoint: {
        local_ssd_ms: 1,
        network_nfs_ms: 8.5,
        size_mb: 1.7,
        label: 'LoRA adapter save (1.7 MB)',
      },
      full_checkpoint: {
        local_ssd_ms: 240,
        network_nfs_ms: 3600,
        size_mb: 720,
        label: 'Full model checkpoint (720 MB)',
      },
      key_insight:
        'LoRA reduces checkpoint I/O by 424x. On network storage, adapter saves complete in <1ms vs 3600ms for full checkpoints.',
    },
  }
  ...
  return (
    <div className="max-w-5xl mx-auto">
        ...
        {metrics.map((m) => (
          <button
            ...
            className={`px-4 py-2 text-sm rounded-md transition-colors ${
              metric === m.id ? 'bg-cyan-600 text-white' : 'text-slate-400 hover:text-slate-200'
            }`}
          >
            {m.label}
          </button>
      ...
      <div className="p-4 rounded-lg bg-slate-800/30 border border-slate-700/50 mb-4">
        {storageView === 'footprint' && <StorageFootprintChart profile={displayProfile} />}
        {storageView === 'timeline' && <IOTimelineChart profile={displayProfile} />}
        {storageView === 'architecture' && <StorageArchitectureChart profile={displayProfile} />}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="p-4 rounded-lg bg-violet-950/20 border border-violet-800/30">
          ...
          <p className="text-xs text-slate-400">
            ...
            than full model checkpoints. At 70B scale, that's 340 MB vs 140 GB per save — the
            difference between milliseconds and minutes on network storage.
          </p>
        </div>
        <div className="p-4 rounded-lg bg-blue-950/20 border border-blue-800/30">
          ...
          <p className="text-xs text-slate-400">
            ...
            SFT adapter, creating a dependency chain on storage throughput.
          </p>
        </div>
        <div className="p-4 rounded-lg bg-emerald-950/20 border border-emerald-800/30">
          ...
          <p className="text-xs text-slate-400">
            ...
            can cause contention with other training jobs.
          </p>
        </div>
      </div>
