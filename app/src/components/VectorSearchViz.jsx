...
const QUERY_POINT = { x: 0.74, y: 0.3, label: 'Query' }
...
export default function VectorSearchViz({ autoPlay = false }) {
  ...
  useEffect(() => {
    ...
    svg
      ...
      .text('Embedding dimension 1 (simplified to 2D)')
    svg
      .append('text')
      .attr('x', 10)
      .attr('y', height / 2)
    ...
    <div>
      ...
      <div className="flex gap-2 mt-2">
        {phase === 0 && (
          <button
            onClick={() => setPhase(1)}
            className="px-4 py-1.5 text-sm bg-blue-700 hover:bg-blue-600 rounded-md transition-colors"
          >
        ...
        {phase >= 4 && (
          <button
            onClick={() => setPhase(1)}
            className="px-4 py-1.5 text-sm bg-slate-700 hover:bg-slate-600 rounded-md transition-colors text-slate-300"
          >
            Replay animation
          </button>
        )}
